...
const disableDevServer = true; // Change it to true if you want to enable dev server.
const filePathPart = disableDevServer ? '_wrapper' : '';
const { clientRoutingBypass, devServerBefore } = require(`./source/dev/auth_login${filePathPart}.js`);

module.exports = function (env,args) {
    if (disableDevServer && env.script === 'start') {
        console.log('Start command is not supported. Please use "npm run build:dev"');
        return;
    }
